...
!!! success "CI/CDによる自動チェック" - GitHub Actionsのワークフローに`sqlfluff lint`および`sqlfluff format --check`を組み込むことで、SQLファイルのフォーマットとスタイルが規約に違反しているコードのマージを自動的にブロックします。- SQLクライアントツールの自動フォーマット機能を活用することを**推奨**します。
