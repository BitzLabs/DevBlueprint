...
*   **ドキュメントコメント (`/** ... */`) の役割:**
    *   本プロジェクトでは、ソースコードの可読性を優先するため、**公開APIに対する`Doxygen` 形式のコメント (`/** ... */`) は原則として使用しません。**
...
*   インクルードガードや`#pragma once`の利用については、**C言語コーディング規約** と同様のルールを適用します。本プロジェクトでは `#pragma once` の利用を推奨します。
