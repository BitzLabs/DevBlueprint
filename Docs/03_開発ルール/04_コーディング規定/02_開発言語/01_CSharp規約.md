...
02_設計仕様/01_API仕様/README.md)** に記述します。
*   **通常のコメント (`//` または `/* ... */`)**:
...
*   詳細なエラーハンドリング戦略は「[エラーハンドリング戦略](../../../02_設計仕様/README.md)」などを参照してください。
...
*   **戻り値の型**:
    ...
    *   **`ValueTask` の注意点:**
        ...
        *   安易に `.Result` や `.GetAwaiter().GetResult()` を呼び出すと、デッドロックや意図しない動作の原因となります。
    *   **`async void` の禁止**:
...
*   **`ConfigureAwait(false)` の利用**:
    *   UIコンテキスト（WPF, WinForms, MAUIなど）やASP.NET CoreのHttpContextに依存しない、**汎用的なライブラリコード**内では、デッドロックを避けるために `await` の後には必ず **`.ConfigureAwait(false)`** を付けます。
